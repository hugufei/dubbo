 ...
 *
 * 实现了ExchangeClient接口，
 * 1)  是基于协议头的【信息交互客户端】实现类
 * 2） 同样它是Client、Channel的适配器
 *
 * 该类主要的作用就是增加了心跳功能
 * 为什么要增加心跳功能呢？
 * 1） 对于长连接，一些拔网线等物理层的断开，会导致TCP的FIN消息来不及发送，对方收不到断开事件，那么就需要用到发送心跳包来检测连接是否断开。
 * 2） consumer和provider断开，处理措施不一样，会分别做出重连和关闭通道的操作。
 *
 */
